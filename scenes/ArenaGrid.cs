...
{
	Blue = 0,   // Tuile alternative Bleue
	Red = 1,    // Tuile alternative Rouge
	Green = 2,  // Tuile alternative Verte
	Yellow = 3  // Tuile alternative Jaune
}
...
{   
	...
	private int totalPlayableCells;
	...
	public void PaintTile(Vector2 globalPosition, TeamColor teamColor)
	{
	...
	private void UpdateUIScores()
	{
		...
		if (ScoreLabelBlue != null) 
			ScoreLabelBlue.Text = Mathf.RoundToInt((float)teamScores[(int)TeamColor.Blue] / totalPlayableCells * 100) + "%";
		
		if (ScoreLabelRed != null) 
			ScoreLabelRed.Text = Mathf.RoundToInt((float)teamScores[(int)TeamColor.Red] / totalPlayableCells * 100) + "%";
		
		if (ScoreLabelGreen != null) 
			ScoreLabelGreen.Text = Mathf.RoundToInt((float)teamScores[(int)TeamColor.Green] / totalPlayableCells * 100) + "%";
		
		if (ScoreLabelYellow != null) 
			ScoreLabelYellow.Text = Mathf.RoundToInt((float)teamScores[(int)TeamColor.Yellow] / totalPlayableCells * 100) + "%";
	}
...
public override void _Process(double delta)
	{
		...
		if (Input.IsMouseButtonPressed(MouseButton.Left))
		{
			...
			PaintTile(mousePos, TeamColor.Blue);
		}
		...
		{
			Vector2 mousePos = GetGlobalMousePosition();
			PaintTile(mousePos, TeamColor.Red);
		}
	}
}
